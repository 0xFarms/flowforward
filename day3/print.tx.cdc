...
transaction() {
  ...
  prepare(auth: AuthAccount) {
    ...
    let myCanvas: Artist.Canvas = Artist.Canvas(width: 5, height: 5, pixels: "0000000000000000000000000")

    // create reference to Artist.Printer
    let artistPrinterRef = getAccount(0x01cf0e2f2f715450) // Artist account needs to be hardcoded in since that's where the Printer is
