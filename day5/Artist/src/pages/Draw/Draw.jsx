  ...
  const onPrint = async () => {
    if (!state.isLoading) {
      ...
      try {
        ...
        const didSucceed = response.events.find((event) => event.type.endsWith(`LocalArtist.PicturePrintSuccess`));
        const didFail = response.events.find((event) => event.type.endsWith(`LocalArtist.PicturePrintFailure`));
        if (didSucceed) {
          dispatch({
            type: 'setMessage',
            payload: {
