...
function Provider(props) {
  ...
  const createCollection = useCallback(
    async () => {
      // 0xfarms Done TODO: Implement the createCollection transaction using "fcl.send".
  ...
  const destroyCollection = useCallback(
    async () => {
      // 0xfarms Done TODO: Implement the destroyCollection.cdc transaction using "fcl.send".
      const transactionId = await fcl
        .send([
          fcl.transaction`
          import LocalArtist from ${process.env.REACT_APP_ARTIST_CONTRACT_HOST_ACCOUNT}
    ...
    async (address) => {
      if (address || state.user.addr) {
        try {
          ...
          // 0xfarms Done TODO: Implement the getCollections.cdc script using "fcl.script", and
  ...
  const printPicture = useCallback(
    async (picture) => {
      // 0xfarms done TODO: Implement the print.cdc transcation using "fcl.send".
  ...
  const logIn = () => {
    // 0xfarms done TODO: Implement FCL log in.
    fcl.logIn(); // Log in or sign up if user doesn't have a wallet.
    // fcl.authenticate();
    // 0xfarms done TODO: Once implemented, remove the "setUser" call.
  };
  const logOut = () => {
    // 0xfarms done TODO: Implement FCL log out.
    fcl.unauthenticate(); // Log out.
  };

  useEffect(() => {
    // 0xfarms done TODO: Implement FCL subscription to get current user.
