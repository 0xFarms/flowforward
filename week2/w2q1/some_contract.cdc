access(all) contract SomeContract {
    ...
    pub struct SomeStruct {
        ...
        pub var b: String // read scope: all (read from anywhere in contract, any script, any transaction). write scope: can only be overwritten in current + inner scope

        access(contract) var c: String	// read scope: anywhere in contract. write scope: current and inner scope only. ie: questsAreFun() cannot overwrite it even though in same contract

        access(self) var d: String	    // read scope/write scope: current and inner scope only
        ...
        pub fun structFunc() { // structFunc() can be accessed by anything with access to SomeStruct
            ...
            // functions with access -  anything can access this function (public/access(all)). can access all functions inside SomeStruct. technically can access any function with "public/access(all)" modifier (using SomeContract.PATH_TO_FUNCTION_CALL).
        }
    ...
    pub resource SomeResource {	    // SomeResource can be created by anything that has access to SomeContract
        ...
        pub fun resourceFunc() {	// resourceFunc() can be accessed by anything that has access to SomeResource
            ...
            // functions with access - anything can access this function (public/access(all)). technically can access any function with "public/access(all)" modifier (using SomeContract.PATH_TO_FUNCTION_CALL).
        }
    ...
    pub fun questsAreFun() {	// can be accessed from anything that has access to SomeContract
        ...
        // log(self.testStruct.d) //doesn't work beacuse "d" is access(self) / private access
