...
pub contract Kibble: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            ...
            let vault: @Kibble.Vault <- from as! @Kibble.Vault
    ...
    pub resource Minter {
        ...
        pub fun mintTokens(amount: UFix64): @Kibble.Vault {
            ...
            pre {
                amount > 0.0: "pre-condition to make sure 'amount' is greater than 0.0"
            }
            // 2) Update Kibble.totalSupply by adding 'amount'
            Kibble.totalSupply = Kibble.totalSupply + amount  // updates ecosystem

            // 3) emit TokensMinted
            emit MinterCreated(allowedAmount: amount)   // amount is num tokens minted

            // 4) return a Vault with balance == 'amount'
            return <- create Vault(balance: amount) // amount is num tokens minted
        }
